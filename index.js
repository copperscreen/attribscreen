...
function clamp(val, maxVal, bits){
	var upper = 1 << (bits);
	var scaledDown = Math.round(upper * val / maxVal);
	return Math.round(maxVal * scaledDown / upper);
}
...
function red1(im){
	for(var i=0; i< im.bitmap.data.length;i+=4){
		im.bitmap.data[i] = 255;
		im.bitmap.data[i+1] = 0;
		im.bitmap.data[i+2] = 0;
		im.bitmap.data[i+3] = 255;
	}
}
...
app.post('/', function(req, res){
	...
	var file = b.url?b:new Buffer(b.file.split(',',2).pop(), 'base64');
	Jimp.read(file).then(image =>{
		...
		image.getBase64Async(image.getMIME()).then(encoded => {
